...
| 设置项 | 说明 | 默认值 |
|--------|------|--------|
| lrclib.net | 开源歌词库，支持同步歌词 | 开启 |
| 网易云音乐 | 通过搜索 API 获取歌词 | 开启 |

### 自定义 API
| 设置项 | 说明 | 默认值 |
|--------|------|--------|
| 启用自定义 API | 使用自定义 API 获取歌词 | 关闭 |
| API 地址 | 自定义 API 的 URL | 空 |
| 请求方式 | GET 或 POST | GET |
...
3. **lrclib.net**（如果启用）- 开源歌词库，支持同步歌词
4. **网易云音乐**（如果启用）- 通过搜索 API + paugram API 获取歌词

你可以在设置中单独开启/关闭每个歌词源。

### 自定义 API

自定义 API 允许你使用自己的歌词服务。启用后，插件会优先从自定义 API 获取歌词，失败时自动回退到启用的内置源。
